...
describe('BrokerageService - Stream Orders', () => {
    ...
    it('should create orders stream for all accounts', async () => {
        const mockEmitter = new EventEmitter();
        mockStreamManager.createStream.mockResolvedValueOnce(mockEmitter);

        const stream = await brokerageService.streamOrders('123456,789012');
        expect(stream).toBe(mockEmitter);
        expect(mockStreamManager.createStream).toHaveBeenCalledWith(
            '/v3/brokerage/stream/accounts/123456,789012/orders',
            undefined,
    ...
    it('should handle stream events correctly', async () => {
        ...
        const stream = await brokerageService.streamOrders('123456');
    ...
    it('should handle network errors', async () => {
        ...
        await expect(brokerageService.streamOrders('123456'))
            .rejects
            .toThrow('Network error');
        expect(mockStreamManager.createStream).toHaveBeenCalledWith(
            '/v3/brokerage/stream/accounts/123456/orders',
            undefined,
            { headers: { 'Accept': 'application/vnd.tradestation.streams.v3+json' } }
        );
    });
